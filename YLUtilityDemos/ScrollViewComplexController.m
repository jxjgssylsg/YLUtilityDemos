...
- (void)viewDidLoad {
    ...
    for (NSUInteger i = 0; i < 10; ++i) {
        ...
        [label mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.height.mas_equalTo(40); // 若注释这句, label 就全部展开了
        }];
        
        lastLabel = label;
        [self.expandStates addObject:[@[label, @(NO)] mutableCopy]]; // 注. 加的是数组
    }
...
- (void)onTap:(UITapGestureRecognizer *)sender {
  ...
  for (NSMutableArray *array in self.expandStates) {
    UILabel *view = [array firstObject]; // 第一个是 label
    ...
    if (view == tapView) {
      ...
      if ([state boolValue] == YES) {
        [view mas_updateConstraints:^(MASConstraintMaker *make) { // mas_updateConstraints
        ...
        if (index - 1 < self.expandStates.count && index >= 1) {
          preView = [[self.expandStates objectAtIndex:index - 1] firstObject];   // 前一个 View
        }
        
        if (index + 1 < self.expandStates.count) {
           nextView = [[self.expandStates objectAtIndex:index + 1] firstObject]; // 后一个 View
        }

        [view mas_remakeConstraints:^(MASConstraintMaker *make) { // 注意. 这里是 remake, 所以会全展开.约束全部从新设置
          if (preView) {
        ...
        if (nextView) {
          [nextView mas_updateConstraints:^(MASConstraintMaker *make) { // mas_updateConstraints
