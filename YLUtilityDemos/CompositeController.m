...
- (void)viewDidLoad {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    [view mas_makeConstraints:^(MASConstraintMaker *make) {
      make.edges.equalTo(lastView).insets(UIEdgeInsetsMake(20, 20, 20, 20)); // 每一个 View 相对于上一个 $$
    }];
...
- (void)onTap {
    ...
    if (self.isNormal) {
        for (NSInteger i = self.viewArray.count - 1; i >= 0; --i) {
            ...
            [self.view bringSubviewToFront:itemView]; // 将 UIView 显示在最前面
            lastView = itemView;
        }
    } else {
    ...
    [UIView animateWithDuration:0.5 animations:^{
        ...
        self.isNormal = !self.isNormal; // 完成后置反
    }];
}
...
- (void)updateViewConstraints { // 这个是后调用的 
    NSLog(@"hahaha updateViewConstraints");
